# -*- coding: utf-8 -*-
"""
股票監控面板 - 富邦 WebSocket 即時價 + TWSE/TPEx 官方報價
...
REFRESH_SEC = 30
HISTORY_CACHE_TTL = 3600  # 盤中 TWSE/TPEx 官方報價每小時最多重抓一次
...
# =============================================================================
# TWSE / TPEx 官方報價：取代 TWSE/TPEx 資料來源
# =============================================================================
...
def parse_twse_item(item: dict):
    """解析 TWSE MIS msgArray 單筆資料。"""
    if not isinstance(item, dict):
        return None
    price = parse_price_value(item.get("z")) or parse_price_value(item.get("pz")) or parse_price_value(item.get("y"))
    previous_close = parse_price_value(item.get("y")) or price
    open_price = parse_price_value(item.get("o")) or price or previous_close
    high_price = parse_price_value(item.get("h")) or price or previous_close
    low_price = parse_price_value(item.get("l")) or price or previous_close
    volume = parse_price_value(item.get("v")) or 0
    name = item.get("n") or item.get("nf") or ""
    return {
        "price": price,
...
def parse_tpex_item(row: dict):
    """解析 TPEx OpenAPI 單筆資料；TPEx 欄位名稱可能隨版本調整，因此採多欄位容錯。"""
    ...
    price = pick("Close", "close", "LatestPrice", "LastPrice", "成交價", "收盤價", "最新價")
    previous_close = pick("PreviousClose", "PreviousClosePrice", "RefPrice", "ReferencePrice", "昨收", "參考價") or price
    open_price = pick("Open", "OpenPrice", "開盤價") or price or previous_close
    high_price = pick("High", "HighPrice", "最高價") or price or previous_close
    low_price = pick("Low", "LowPrice", "最低價") or price or previous_close
    volume = pick("TradingShares", "TradeVolume", "Volume", "成交股數", "成交量") or 0
    name = row.get("CompanyName") or row.get("SecuritiesCompanyName") or row.get("Name") or row.get("名稱") or ""
    return {
        "price": price,
        "previous_close": previous_close,
        "open": open_price,
...
def get_twse_tpex_quote(symbol: str):
    """依使用者指定邏輯：先查 TWSE MIS，無 msgArray 再查 TPEx OpenAPI。"""
    stock_id = symbol_to_code(symbol)
    ...
    try:
        ...
        twse = {"msgArray": [], "error": str(e)}

    msg_array = twse.get("msgArray") if isinstance(twse, dict) else []
    use_tpex = not msg_array

    if not use_tpex:
        item = msg_array[0]
        parsed = parse_twse_item(item)
        if parsed and parsed.get("price") is not None:
            return {"success": True, "market": "TWSE", **parsed}
        return {"success": False, "msg": "TWSE無可用價格", "raw": twse}

    if use_tpex and not stock_id.startswith("^"):
        try:
            url = "https://www.tpex.org.tw/openapi/v1/tpex_mainboard_quotes"
            data = session.get(url, timeout=5).json()
            row = next((x for x in data if str(x.get("SecuritiesCompanyCode", "")).strip() == stock_id), None)
            if not row:
                return {"success": False, "msg": "TPEx無資料", "raw": data[:3] if isinstance(data, list) else data}
            parsed = parse_tpex_item(row)
            if parsed and parsed.get("price") is not None:
                return {"success": True, "market": "TPEx", **parsed}
            return {"success": False, "msg": "TPEx無可用價格", "raw": row}
        except Exception as e:
            return {"success": False, "msg": f"TPEx連線失敗：{e}"}

    return {"success": False, "msg": "TWSE/TPEx皆無資料"}


@st.cache_data(ttl=HISTORY_CACHE_TTL)
def download_stock_data(symbol):
    """
    使用 TWSE/TPEx 官方報價建立運算基準資料。

    注意：使用者指定的 TWSE MIS / TPEx OpenAPI 是即時/當日報價來源，
    不提供 3 個月 OHLC 歷史 K 線。因此此函式會以昨收 previous_close 建立
    『昨日基準列』，讓漲跌幅與即時價運算可以正常顯示；MA/KD/MACD 在資料不足時會顯示資料不足。
    """
    quote = get_twse_tpex_quote(symbol)
    if not quote.get("success"):
        raise ValueError(quote.get("msg", "TWSE/TPEx 無法取得資料"))

    today = datetime.now(TW_TZ).date()
    prev_date = pd.Timestamp(today) - pd.Timedelta(days=1)
    prev_close = quote.get("previous_close") or quote.get("price")
    if prev_close is None:
        raise ValueError(f"TWSE/TPEx 無法取得 {symbol} 昨收/參考價")
...
def is_after_1330():
    """13:30 後不自動抓 TWSE/TPEx，只有手動更新才會重新抓。"""
    now = datetime.now(TW_TZ).time()
    end = datetime.strptime("13:30", "%H:%M").time()
    return now >= end
...
@st.cache_data(ttl=30)
def get_official_latest_price(symbol: str):
    quote = get_twse_tpex_quote(symbol)
    if quote.get("success") and quote.get("price") is not None:
        return float(quote["price"]), quote.get("source", "TWSE/TPEx")
    raise ValueError(quote.get("msg", f"TWSE/TPEx 無法取得 {symbol} 價格"))

@st.cache_data(ttl=30)
def get_yahoo_tw_quote_price(symbol: str):
    """保留原函式名稱相容舊流程；實際資料來源改為 TWSE/TPEx。"""
    return get_official_latest_price(symbol)

@st.cache_data(ttl=300)
def get_official_latest_daily_close(symbol: str):
    quote = get_twse_tpex_quote(symbol)
    if quote.get("success") and quote.get("price") is not None:
        return float(quote["price"]), datetime.now(TW_TZ).date(), quote.get("source", "TWSE/TPEx")
    raise ValueError(quote.get("msg", f"TWSE/TPEx 無法取得 {symbol} 最新價格"))
...
def after_1330_price_logic(symbol, df, forced=False):
    ...
    try:
        official_price, official_source = get_official_latest_price(symbol)
    ...
    try:
        daily_close, _, source = get_official_latest_daily_close(symbol)
        if daily_close is not None and pd.notna(daily_close):
            return float(daily_close), ("Forced 13:30 " + source) if forced else (source + " daily after 13:30")
    except Exception:
        pass
...
def get_last_price(symbol, df, manager=None):
    ...
    # 若尚未收到 WebSocket 價格，則用「已快取的歷史收盤價」暫代，不再呼叫其他即時報價。
    if use_fubon_ws:
...
def get_stock_name(symbol: str) -> str:
    ...
    quote = get_twse_tpex_quote(symbol)
    if quote.get("success") and quote.get("name"):
        return str(quote.get("name")).strip()
    return code
...
def compute_indicators(df, price):
    ...
    # 使用者指定的 TWSE/TPEx 來源是即時報價 API，不是歷史 K 線 API。
    # 若資料不足 26 筆，仍回傳價格與漲跌幅，技術指標標示為資料不足，避免整個畫面中斷。
    if len(calc_df) < 26:
        gap_signal = "-"
        if ENABLE_GAP_SIGNAL and pd.notna(yesterday_high) and price_val > yesterday_high:
            gap_signal = "跳空"
        return {
...
def render_fubon_login():
    ...
    if FubonSDK is None:
        st.sidebar.warning("富邦 SDK 未載入，當日價格會使用 TWSE/TPEx fallback。")
        return
    ...
    if not pfx_base64:
        st.sidebar.warning("未設定 st.secrets['fubon']['pfx_base64']，當日價格會使用 TWSE/TPEx fallback。")
        return
...
def should_rebuild_market_snapshot(signature: str, can_push_now: bool = False) -> tuple[bool, str]:
    """
    重建行情快照規則：
    1. 盤中 09:00~13:30：畫面可每 REFRESH_SEC 重算一次，但 TWSE/TPEx 歷史資料由 @st.cache_data(ttl=3600) 控制每小時最多抓一次。
    2. 13:30 後：不自動重抓，只有按「手動更新」才重抓 TWSE/TPEx / Yahoo TW。
    ...
    # 盤前也不要自動打 TWSE/TPEx；若需要盤前資料，按手動更新。
    ...
    # 盤中自動更新：只重算快照與 WebSocket 即時價；TWSE/TPEx 歷史資料因 TTL=3600 不會一直打。
...
with st.sidebar.expander("🕒 價格來源模式", expanded=True):
    ...
    elif current_mode == "official":
        st.info("目前價格模式：強制 官方報價，抓值邏輯同 13:30 後。再次按 官方報價 可回到自動模式。")
    else:
        if is_fubon_realtime_time():
            st.info("目前價格模式：自動；09:00~13:30 優先 WebSocket；TWSE/TPEx 官方報價每小時快取")
        else:
            st.info("目前價格模式：自動；13:30 後停止自動抓資料，需按手動更新；資料源 TWSE/TPEx")
    ...
    with mode_col1:
        ...
        if st.button("WebSocket", key="force_websocket_price_btn", width="stretch", type=ws_button_type):
            ...
            st.rerun()
    with mode_col2:
        yf_button_type = "primary" if current_mode == "official" else "secondary"
        if st.button("官方報價", key="force_TWSE/TPEx_price_btn", width="stretch", type=yf_button_type):
            st.session_state.price_source_override = "auto" if current_mode == "official" else "TWSE/TPEx"
            st.rerun()
...
if st.session_state.market_snapshot_at:
    st.caption(
        ...
        f"TWSE/TPEx報價TTL：{HISTORY_CACHE_TTL // 60}分鐘"
    )
else:
